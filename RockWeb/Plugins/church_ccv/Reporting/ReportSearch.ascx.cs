...
namespace RockWeb.Plugins.church_ccv.Reporting
{
    ...
    [Description( "Shows the results of a Report search" )]
    ...
    public partial class ReportSearch : RockBlock
    {
        ...
        private void BindGrid()
        {
            ...
            string reportName = PageParameter( "ReportName" );

            var qry = reportService.Queryable().Where( a => a.Name.Contains( reportName ) );

            if ( qry.Count() == 1 )
            {
                var report = qry.First();
                var qryParams = new Dictionary<string, string>();
                qryParams.Add( "ReportId", report.Id.ToString() );
                qryParams.Add( "ExpandedIds", CategoryExpandedIds( CategoryCache.Read( report.CategoryId ?? 0 ) ));
                NavigateToLinkedPage( "ReportPage", qryParams );
            }
            else
            {
        ...
        protected void gList_RowDataBound( object sender, GridViewRowEventArgs e )
        {
            ...
            var lReportPath = e.Row.ControlsOfTypeRecursive<Literal>().FirstOrDefault( a => a.ID == "lReportPath" );
            if ( report != null && lReportPath != null && report.CategoryId.HasValue )
            {

                var category = CategoryCache.Read( report.CategoryId.Value );
                lReportPath.Text = CategoryAncestorPathName( category );
            }
        }
        ...
        protected void gList_RowSelected( object sender, RowEventArgs e )
        {
            RockContext rockContext = new RockContext();
            ReportService reportService = new ReportService( rockContext );
            var report = reportService.Get( e.RowKeyId );
