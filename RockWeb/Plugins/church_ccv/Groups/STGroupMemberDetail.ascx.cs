...
{
    [DisplayName( "ST Group Member Detail" )]
    [Category( "CCV > Groups" )]
    [Description( "Displays the details of the given Short Term group member " )]
    [WorkflowTypeField( "OptOut Completed Group", "The workflow to use when opting out a person due to the group being completed. The Person will be set as the workflow 'Entity' attribute when processing is started.", false, false, "", "" )]
    ...
    public partial class STGroupMemberDetail : ToolboxGroupMemberDetail, IDetailBlock
    {
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            OptOutReasonDefinedType = DefinedTypeCache.Read( new Guid( church.ccv.Utility.SystemGuids.DefinedType.SHORT_TERM_OPT_OUT_REASON ) );
        }
        ...
        private void SaveGroupMember()
        {
            if ( Page.IsValid )
            {
                ...
                {
                    case church.ccv.Utility.SystemGuids.DefinedValue.SHORT_TERM_OPT_OUT_COMPLETED_GROUP:
                    {
                        StartWorkflow( "OptOutCompletedGroup", groupMember.Person, workflowAttribs, rockContext );
                        break;
                    }

                    case church.ccv.Utility.SystemGuids.DefinedValue.SHORT_TERM_OPT_OUT_DID_NOT_SHOW:
                    {
                        StartWorkflow( "OptOutDidNotShow", groupMember.Person, workflowAttribs, rockContext );
                        break;
                    }
                }
            }
        ...
        {
            // follow up date is never needed for Short Term groups.
            dpFollowUpDate.Visible = false;
