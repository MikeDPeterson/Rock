...
{
    [DisplayName( "ST Group Detail Lava" )]
    [Category( "CCV > Groups" )]
    ...
    public partial class STGroupDetailLava : ToolboxGroupDetailLava
    {
        ...
        const int ST_GroupRole_Attendee = 159;

        protected override void OnInit( EventArgs e )
        {
            base.OnInit( e );

            ExcludedAttribKeys.Add( "GroupFocus" );
        }
        ...
        protected override void HandlePageAction( string action, string parameters )
        {
            switch ( action )
            {
                ...
                case "SendCommunicationActive":
                {
                    var personAliasIds = new GroupMemberService( new RockContext( ) ).Queryable()
                                    .Where( m => m.GroupId == _groupId && m.GroupMemberStatus == GroupMemberStatus.Active )
                                    .ToList()
                ...
                case "SendCommunicationPending":
                {
                    var personAliasIds = new GroupMemberService( new RockContext( ) ).Queryable()
                                    .Where( m => m.GroupId == _groupId && m.GroupMemberStatus == GroupMemberStatus.Pending )
                                    .ToList()
                                    .Select( m => m.Person.PrimaryAliasId )
                                    .ToList();
        ...
        protected void btnSaveGroupMember_Click( object sender, EventArgs e )
        {
            if( Page.IsValid && ppGroupMemberPerson.SelectedValue.HasValue )
            {
                ...
                bool result = AddGroupMember( ppGroupMemberPerson.SelectedValue.Value, _groupId, ST_GroupRole_Attendee, selectedStatus.Value );
